...
//  console.log(waypts);
  ...
  if (document.getElementById('index')){
      var transferFromValid = false;
      var transferToValid = false;
      var chaffeurValid = false;
      
      document.getElementById('cform').addEventListener('submit', function(e){
        if ($('#pac-input-from-chaffeur').val() == '' || chaffeurValid == false) {
            e.preventDefault()
   ...
   document.getElementById('tform').addEventListener('submit', function(e){
       if ($('#from').val() == '' || transferFromValid == false) {
           e.preventDefault();
            $('#from').addClass('error');
            $(window).scrollTop(0);
            
       }else if ($('#to').val() == '' || transferToValid == false) {
            e.preventDefault();
            $('#to').addClass('error');
            $(window).scrollTop(0);
...
if(document.getElementById('chaffeurForm')){
  ...
  function updatePriceChaffeurBox(distance){
      ...
      var rate = priceInFixedBox.dataset.rate;
      
      var carPrice = Number(priceInFixedBox.dataset.transferCarPrice) * Number(priceInFixedBox.dataset.rate);
...
//                         $('.time-picker.to').trigger('dp.change');
            ...
            var selected = $('.date-picker').datepicker("getDate");
            var timeControls = document.getElementsByClassName('time-control')
            
            if (selected == null) {
                $(timeControls[i]).find('.date').text('')
            } else {
...
try{
 ...
 function displayMapSummary(){
     ...
     if (document.getElementById('confirm-chaffer')){
         ...
         var mapProp = {
            center: new google.maps.LatLng(40.435, 49.8676),
            zoom:9,
